...
| **Multijugador (Fase 2)** | Sincronización en tiempo real de jugadores con Socket.IO en mapas. | **Implementado en Sprint 2**. Sincronización de posición, armas y acciones de otros jugadores concurrentes. Sistema de party en memoria. | Chat global del mundo y emotes. |
...
| **Integración con APIs (Google Fit)** | Carga automática de actividad real y cardio. | **Implementado en Sprint 2**. Vinculación OAuth2 con Fitbit (Google Health) integrada. Sincroniza pasos y ejercicios reales. | Completado en su totalidad. |
...
### 3. Rediseño de la Penalización por Inactividad (Fisiología Real vs Retención)
* **El Problema (Punición Extrema)**: La penalización por inactividad prolongada (> 21 días) reduce un 30% todas las estadísticas y quita 2 niveles al personaje. Aunque esta severidad representa la realidad biológica de la pérdida de masa muscular y acondicionamiento cardiovascular por descuido físico, en términos de retención de jugadores (*game design*) puede causar que un jugador que se ausentó por viaje o enfermedad abandone el juego de manera definitiva debido a la frustración de la pérdida masiva.
* **La Solución Implementada (Fisiología Asimétrica y Estado "Oxidado")**: Se implementó un sistema de penalización asimétrico basado en la fisiología del cuerpo humano y el entrenamiento cerebral:
  - **Decaimiento Asimétrico**: Las estadísticas físicas (Fuerza, Resistencia, Destreza, Velocidad) sufren una atrofia severa del 30%, representando la pérdida real de masa muscular y capacidad VO2 max. Las estadísticas mentales (Inteligencia, Maná Máximo) sufren una pérdida menor del 10%, ya que el conocimiento se oxida pero no se pierde al mismo ritmo. Las estadísticas de voluntad y carisma decaen un 20%. El nivel del jugador permanece intacto.
  - **Debuff de Estado "Oxidado" (Recovery)**: Mientras el personaje se encuentra en recuperación (`inRecovery`), los hechizos cuestan un 20% más de maná, los ataques físicos hacen un 20% menos de daño y la Vitalidad se limita a un máximo de 100 (en vez de 300). Esto representa la rigidez muscular de volver al gimnasio tras un largo descanso.
  - **Recuperación Rápida**: Para restaurar las estadísticas originales guardadas en el respaldo, el jugador solo necesita registrar **3 actividades físicas reales** (fuerza, cardio, intenso o correr) en lugar de completar un ciclo de consistencia de 30 días.
